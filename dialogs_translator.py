...
from src.print_and_log import log
from src.print_neatly import print_neatly
from src.translator.translator import translate as tr_manager


def translate(file_path, src='it', dst='en', verbose=False, max_retries=5):

    def translate_sentence(text):
        target = text
        translation = tr_manager(target, src, dst)
        if target[0].isalpha() and translation[0].isalpha and not target[0].isupper():
            translation = translation[0].lower() + translation[1:]
        text = translation
        if verbose:
            log(target, '->', translation)
        return text
    ...
    for events in data["events"]:
        if events is not None:
            log('{}: {}/{}'.format(file_path, i+1, num_events))
            i += 1
                ...
                for list in pages['list']:
                    ...
                    if list['code'] == 401:
                        ...
                        if not list['parameters'][0]:
                            log('Anomaly plain text - Unexpected 401 code: {}'.format(list['parameters'][0]))
                            continue
                        # translate
                        list['parameters'][0], success = try_translate_sentence(list['parameters'][0])
                        if not success:
                            log('Anomaly plain text: {}'.format(list['parameters'][0]))
                        else:
                            translations += 1
                    # Choices (ex: [["yes", "no"], 1, 0, 2, 0])
                        ...
                        for j, choice in enumerate(list['parameters'][0]):
                            # null or empty string check
                            if not choice:
                                log('Anomaly choices - Unexpected 102 code: {}'.format(choice))
                                continue
                            # translate
                            list['parameters'][0][j], success = try_translate_sentence(choice)
                            if not success:
                                log('Anomaly choices: {}'.format(choice))
                            else:
                                translations += 1
                    # Choices (answer) (ex: [0, "yes"])
                        ...
                        if len(list['parameters']) != 2 or not list['parameters'][1]:
                            log('Anomaly choices (answer) - Unexpected 402 Code: {}'.format(list['parameters']))
                            continue
                        # translate
                        list['parameters'][1], success = try_translate_sentence(list['parameters'][1])
                        if not success:
                            log('Anomaly choices (answer): {}'.format(list['parameters'][1]))
                        else:
                            translations += 1
...
def translate_neatly(file_path, src='it', dst='en', verbose=False, max_len=40, max_retries=5):
                ...
                while list_it < len_list:
                    ...
                    # 402 Choices (answer) (dont nestly translate) (ex: [0, "yes"])
                        ...
                        if len(pages['list'][list_it]['parameters']) != 2 or not pages['list'][list_it]['parameters'][1]:
                            log('Anomaly choices (answer) - Unexpected 402 Code: {}'.format(pages['list'][list_it]['parameters']))
                            list_it += 1
                            continue
                        # translate
                        pages['list'][list_it]['parameters'][1], success = try_translate_sentence(pages['list'][list_it]['parameters'][1])
                        if not success:
                            log('Anomaly choices (answer): {}'.format(pages['list'][list_it]['parameters'][1]))
                        else:
                            translations += 1
                        list_it += 1
                    ...
                    # 401 Plain text (to nestly translate) (ex: ["plain text"])
                        ...
                        # empty string check
                        if not text:
                            log('Anomaly (empty text): {}'.format(text))
                            list_it = list_it_2
                            continue
                        # translate
                        text_tr = None
                        ...
                        if text_tr is None:
                            ...
                            for text_it, j in enumerate(range(list_it, list_it_2)):
                                ...
                                if text_it >= len(text_neat):  # translated text is one row shorter
                                    text_neat.append("")
                                if verbose:
                                    log(pages['list'][j]['parameters'][0], "->", text_neat[text_it])
...
def translate_neatly_common_events(file_path, src='it', dst='en', verbose=False, max_len=55, max_retries=5):
    ...
    for d in data:
        if d is not None:
            log('{}: {}/{}'.format(file_path, i+1, num_ids))
            i += 1
            list_it = 0
            len_list = len(d['list'])
            while list_it < len_list:
                    ...
                    if text_tr is None:
                        ...
                        for text_it, j in enumerate(range(list_it, list_it_2)):
                            ...
                            if text_it >= len(text_neat):
                                text_neat.append("")
                            if verbose:
                                log(d['list'][j]['parameters'][0], "->", text_neat[text_it])
...
if __name__ == '__main__':
    ...
    for file in os.listdir(args.input_folder):
        ...
        if os.path.isfile(os.path.join(dest_folder, file)):
            log('skipped file {} because it has already been translated'.format(file_path))
            continue
        if file.endswith('.json'):
            log('translating file: {}'.format(file_path))
            if file.startswith('Map'):
                if args.print_neatly:
                    new_data, t = translate_neatly(file_path, max_len=args.max_len,
                                                   src=args.source_lang, dst=args.dest_lang, verbose=args.verbose,
                                                   max_retries=args.max_retries)
                else:
                    new_data, t = translate(file_path,
                                            src=args.source_lang, dst=args.dest_lang, verbose=args.verbose,
                                            max_retries=args.max_retries)
            elif file.startswith('CommonEvents'):
                new_data, t = translate_neatly_common_events(file_path, max_len=args.max_len,
                                               src=args.source_lang, dst=args.dest_lang, verbose=args.verbose,
                                               max_retries=args.max_retries)
            translations += t
